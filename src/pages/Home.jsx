...
import { Mail, Instagram, Twitter, Globe, Home, Folder, Gift, Key, Edit } from 'lucide-react';
...
const HomePage = () => {
  ...
  const premiumTools = [
    { name: 'Framer', description: 'Website builder', icon: '/api/placeholder/48/48' },
    { name: 'Figma', description: 'Design Tool', icon: '/api/placeholder/48/48' },
    { name: 'Lemon Squeezy', description: 'Payments Provider', icon: '/api/placeholder/48/48' },
    { name: 'ChatGPT', description: 'AI Assistant', icon: '/api/placeholder/48/48' },
    { name: 'Notion', description: 'Productivity Tool', icon: '/api/placeholder/48/48' },
    { name: 'Next.js', description: 'React Framework', icon: '/api/placeholder/48/48' }
  ];
  ...
  return (
    <div className="min-h-screen text-white bg-black">
      <div className="/*fixed*/ top-0 left-0 right-0 z-50 flex items-center justify-center w-full p-6 bg-black/50 backdrop-blur-sm">
        <div className="flex gap-8 px-6 py-2 bg-orange-500 bg-opacity-10 rounded-xl">
          <div className="relative group">
            <Home className="w-6 h-6 cursor-pointer hover:text-gray-400" />
            <span className="absolute px-2 py-1 mt-2 text-xs text-white transition-opacity duration-300 transform -translate-x-1/2 bg-orange-500 rounded opacity-0 bg-opacity-20 left-1/2 group-hover:opacity-100">Home</span>
          </div>
          <div className="relative group">
            <Folder className="w-6 h-6 cursor-pointer hover:text-gray-400" />
            <span className="absolute px-2 py-1 mt-2 text-xs text-white transition-opacity duration-300 transform -translate-x-1/2 bg-orange-500 rounded opacity-0 bg-opacity-20 left-1/2 group-hover:opacity-100">Files</span>
          </div>
          <div className="relative group">
            <Gift className="w-6 h-6 cursor-pointer hover:text-gray-400" />
            <span className="absolute px-2 py-1 mt-2 text-xs text-white transition-opacity duration-300 transform -translate-x-1/2 bg-orange-500 rounded opacity-0 bg-opacity-20 left-1/2 group-hover:opacity-100">Gift</span>
          </div>
          <div className="relative group">
            <Key className="w-6 h-6 cursor-pointer hover:text-gray-400" />
            <span className="absolute px-2 py-1 mt-2 text-xs text-white transition-opacity duration-300 transform -translate-x-1/2 bg-orange-500 rounded opacity-0 bg-opacity-20 left-1/2 group-hover:opacity-100">Key</span>
          </div>
          <div className="relative group">
            <Edit className="w-6 h-6 cursor-pointer hover:text-gray-400" />
            <span className="absolute px-2 py-1 mt-2 text-xs text-white transition-opacity duration-300 transform -translate-x-1/2 bg-orange-500 rounded opacity-0 bg-opacity-20 left-1/2 group-hover:opacity-100">Write</span>
          </div>
        </div>
      </div>
...
<svg  className='absolute -rotate-32 w-80 -left-48 top-40' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 252 151">
  ...
  <path d="M 214.46 49.155 C 214.434 50.375 214.624 51.486 215.789 52.281 C 215.727 51.923 215.651 51.569 215.607 51.21 C 215.405 49.569 215.563 47.99 216.663 46.586 C 217.282 45.793 218.085 45.181 219.077 44.671 C 218.994 44.926 218.906 45.124 218.867 45.33 C 218.695 46.229 219.026 47.008 219.72 47.636 C 220.23 48.097 220.818 48.494 221.404 48.882 C 222.446 49.574 223.299 50.393 223.744 51.496 C 224.804 54.128 223.67 57.009 220.698 58.238 C 217.916 59.387 214.327 58.156 213.204 55.586 C 212.263 53.431 212.508 51.348 214.109 49.454 C 214.176 49.375 214.252 49.302 214.328 49.229 C 214.346 49.209 214.377 49.199 214.46 49.155 Z M 215.674 56.191 C 216.438 57.132 218.399 57.803 220.214 57.171 C 222.084 56.52 223.155 54.755 222.759 52.977 C 222.451 51.599 221.086 50.427 219.9 50.272 C 220.647 51.154 220.96 52.123 220.771 53.21 C 220.595 54.231 219.558 55.568 218.832 55.667 C 219.195 54.967 219.402 54.25 219.214 53.469 C 218.623 54.978 217.471 55.899 215.674 56.191 Z" fill="rgb(255,255,255)"></path>
          ...
          <div className="flex-1 p-4 md:p-8 md:max-h-screen md:overflow-y-auto no-scrollbar">
            <div className="mb-12">
              <h1 className="mt-8 -mb-1 text-5xl font-bold text-center md:text-left lg:text-7xl md:text-5xl">SOFTWARE</h1>
              <h2 className="mb-6 text-5xl font-bold text-center text-gray-600 md:text-left lg:text-7xl md:text-5xl">ENGINEER</h2>
              <p className="mb-8 font-semibold text-center text-gray-400 md:text-left">
                Passionate about creating intuitive and engaging user experiences. 
                Specialize in transforming ideas into beautifully crafted products.
              </p>

              <div className="grid grid-cols-3 gap-4 mb-8 md:grid-cols-3 ">
                {stats.map((stat, index) => (
                  <div key={index}>
                    <div className="mb-1 text-5xl font-bold text-center">{stat.value}</div>
                    <div className="text-sm font-semibold text-center text-gray-400">{stat.label}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="mb-16">
              <h2 className="mb-2 text-4xl font-bold">PREMIUM</h2>
              <h3 className="mb-8 text-4xl font-bold text-gray-600">TOOLS</h3>
              
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
                {premiumTools.map((tool, index) => (
                  <div key={index} className="flex items-center gap-3 p-4 transition-colors rounded-xl hover:bg-gray-900">
                    <img src={tool.icon} alt={tool.name} className="w-12 h-12 rounded-xl" />
                    <div>
            ...
            <div className="mb-16">
              <h2 className="mb-2 text-4xl font-bold">DESIGN</h2>
              <h3 className="mb-8 text-4xl font-bold text-gray-600">THOUGHTS</h3>
              
              <div className="space-y-8">
                {designThoughts.map((post, index) => (
                  <div key={index} className="cursor-pointer group">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xl font-semibold group-hover:text-gray-300">{post.title}</h4>
